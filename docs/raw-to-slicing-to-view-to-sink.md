...
**인덱싱용 View 병합** (`ShipWorkflow`):

**프로세스**:
1. **Slice 조회**: `sliceRepository.getByVersion()` → 모든 SliceType 조회
2. **병합**: `mergeSlices()` → Slice들을 하나의 JSON 문서로 병합
3. **Tombstone 필터링**: 삭제된 Slice는 제외

**병합 로직** (`ShipWorkflow.mergeSlices()`):
```kotlin
...
2. `OutboxPollingWorker`가 `SinkRuleRegistry.findByEntityAndSliceType()` 호출
3. 매칭되는 SinkRule마다 `ShipRequested` outbox 생성
4. `ShipEventHandler`가 `ShipWorkflow.execute()` 호출
5. `ShipWorkflow`가 `mergeSlices()` → View 문서 생성
