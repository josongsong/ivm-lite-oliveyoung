# Product RawData → IVM-Lite Contract — DX 제안
...
```bash
# 경로 추출 (PathExpr)
./gradlew extractJsonPaths --sample=.tmp/product/UA30953620.json --output=.tmp/paths.yaml
# → options[*].gdsSelprcUprc, displayCategories[*].sclsCtgrNo 등

# 슬라이스 추천 (초안)
./gradlew pathsToImpactMap --paths=.tmp/paths.yaml --output=.tmp/impact-map-draft.yaml
```
