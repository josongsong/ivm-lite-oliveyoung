...
class DefaultAlertRuleLoader : AlertRuleLoaderPort {
    ...
    private fun loadDefaultRules() {
        rules.addAll(listOf(
            // P0: SinkEvent 실패 누적 (DLQ 대체)
            AlertRule(
                id = "dlq-threshold",
                name = "SinkEvent 실패 누적",
                description = "처리 실패한 SinkEvent가 10개 이상 누적되었습니다.",
            ...
            AlertRule(
                ...
                condition = AlertCondition.RateCheck(
                    metricName = "sink_event.failed.rate",
                    windowSeconds = 60,
                    operator = ThresholdOperator.GT,
                    value = 0.1
                ),
            ...
            AlertRule(
                ...
                name = "Pending 큐 과다",
                description = "처리 대기 중인 SinkEvent가 1000개를 초과했습니다. 처리 지연이 발생할 수 있습니다.",
                condition = AlertCondition.Threshold(
                    metricName = "sink_event.pending.count",
                    operator = ThresholdOperator.GT,
                    value = 1000.0
                ),
                severity = AlertSeverity.WARNING,
